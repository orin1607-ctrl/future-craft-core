...
export async function loadCompanyInsuranceRedStats(companyName: string): Promise<CompanyInsuranceRedStats> {
  const base = supabase.from('vehicles').select('id', { count: 'exact', head: true }).eq('company_name', companyName);
...
/** Bulk update red highlight for all vehicles of one company. Does not touch insurance_alerts_enabled. */
