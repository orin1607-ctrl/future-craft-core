...
describe('priced-order detection and worker approval', () => {
  ...
  it('keeps the garage mailbox separate from Claims and never calls claims-gmail', () => {
    ...
    const fnPath = resolve('supabase/functions/garage-gmail/index.ts');
    const helperPath = resolve('public/oauth/garage-gmail-connect.html');
    const applyPath = resolve('scripts/apply-garage-gmail-staging.mjs');
    const callbackPath = resolve('public/oauth/google-callback.html');
    ...
    if (existsSync(fnPath)) {
      ...
      expect(fnSrc).toContain('eq("connected_email", ALLOWED_ACCOUNT)');
    ...
    if (existsSync(callbackPath)) {
      const callbackSrc = readFileSync(callbackPath, 'utf8');
