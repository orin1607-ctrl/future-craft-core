...
--   no garage_staff table (deferred)
...
--   REVOKE sequences from PUBLIC/anon/authenticated (no direct nextval)
--   numbering functions: none exposed; numbers assigned by SECURITY DEFINER
--     BEFORE INSERT triggers only
--   GRANT SELECT,INSERT,UPDATE on tables (no DELETE)
--   RLS phase 1: super_admin only
--   fleet_manager / driver / garage_photographer / claims worker: blocked
--   composite FK: garage_cases (vehicle_id, customer_id)
--                 → garage_vehicles (id, customer_id)
--   immutable after insert: customer_number, case_number, opened_by
