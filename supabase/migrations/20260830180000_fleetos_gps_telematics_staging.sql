...
-- FleetOS telematics gps_* tables. Does not alter vehicles / faults / accidents / expenses.
-- Rollback: DROP TABLE gps_raw, gps_events, gps_positions, gps_live, gps_can_maps,
--             gps_device_assignments, gps_devices CASCADE;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.gps_device_assignments TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.gps_live TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.gps_positions TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.gps_events TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.gps_raw TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.gps_can_maps TO authenticated;

DROP POLICY IF EXISTS gps_devices_select ON public.gps_devices;
DROP POLICY IF EXISTS gps_devices_write ON public.gps_devices;
DROP POLICY IF EXISTS gps_assignments_select ON public.gps_device_assignments;
DROP POLICY IF EXISTS gps_assignments_write ON public.gps_device_assignments;
DROP POLICY IF EXISTS gps_live_select ON public.gps_live;
DROP POLICY IF EXISTS gps_live_write ON public.gps_live;
DROP POLICY IF EXISTS gps_positions_select ON public.gps_positions;
DROP POLICY IF EXISTS gps_positions_write ON public.gps_positions;
DROP POLICY IF EXISTS gps_events_select ON public.gps_events;
DROP POLICY IF EXISTS gps_events_write ON public.gps_events;
DROP POLICY IF EXISTS gps_raw_select ON public.gps_raw;
DROP POLICY IF EXISTS gps_raw_write ON public.gps_raw;
DROP POLICY IF EXISTS gps_can_maps_select ON public.gps_can_maps;
DROP POLICY IF EXISTS gps_can_maps_write ON public.gps_can_maps;
...
CREATE POLICY gps_live_select ON public.gps_live FOR SELECT TO authenticated
  USING (company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role));
CREATE POLICY gps_live_write ON public.gps_live FOR ALL TO authenticated
  USING (company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role))
  WITH CHECK (company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role));

CREATE POLICY gps_positions_select ON public.gps_positions FOR SELECT TO authenticated
  USING (company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role));
CREATE POLICY gps_positions_write ON public.gps_positions FOR ALL TO authenticated
  USING (company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role))
  WITH CHECK (company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role));

CREATE POLICY gps_events_select ON public.gps_events FOR SELECT TO authenticated
  USING (company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role));
CREATE POLICY gps_events_write ON public.gps_events FOR ALL TO authenticated
  USING (company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role))
  WITH CHECK (company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role));
...
CREATE POLICY gps_raw_select ON public.gps_raw FOR SELECT TO authenticated
  USING (
    ...
    OR company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role)
  );
CREATE POLICY gps_raw_write ON public.gps_raw FOR ALL TO authenticated
  USING (
    company_name IS NULL
    OR company_name = get_user_company(auth.uid()) OR has_role(auth.uid(), 'super_admin'::app_role)
  )
