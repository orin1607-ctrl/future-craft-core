...
function goT(n){
  ...
  if(n===4) buildReport();
  if(n===5){ buildFinal(); mountAllGatePanels(); }
  window.scrollTo(0,0);
}
function assertGatesForBuild(){
  ...
  if(window.PreBuildWorkReport&&!PreBuildWorkReport.assertBuildGate())return false;
  return true;
}
function scrollGate(rootId){
  var el=document.getElementById(rootId);
  if(el&&el.scrollIntoView){el.scrollIntoView({behavior:'smooth',block:'start'});}
  else if(typeof showToast==='function')showToast('⚠️ מודול לא נמצא');
}
function mountAllGatePanels(){
  collect();
  var exported=document.getElementById('exported');
  if(exported)exported.style.display='block';
  if(window.BusinessSummaryApproval&&BusinessSummaryApproval.mountPanel){
    BusinessSummaryApproval.mountPanel('business-summary-approval-root');
  }
  ...
  if(window.SeoStrategy&&SeoStrategy.mountPanel){
    SeoStrategy.mountPanel('seo-strategy-root');
  }
  ...
  if(window.GooglePageQualityStandard&&GooglePageQualityStandard.mountPanel){
    GooglePageQualityStandard.mountPanel('google-readiness-root');
  }
  if(window.AiStrategyRoom&&AiStrategyRoom.mountPanel){
    AiStrategyRoom.mountPanel('ai-strategy-room-root');
  }
...
function exportData(){
  ...
  showToast('🚀 הועבר לעוזרים — אשרו שאלון, חומרים, SEO ודוח לפני בניית אתר');
}

function openWebsiteBuilder(){
  if(!assertGatesForBuild())return;
...
function showToast(m){
  ...
  window.mountAllGatePanels = mountAllGatePanels; window.scrollGate = scrollGate; window.assertGatesForBuild = assertGatesForBuild;
