...
function openWebsiteBuilder(){
  if(window.BusinessSummaryApproval && !BusinessSummaryApproval.assertGate()) return;
  if(window.StrategicBriefing && !StrategicBriefing.assertGate()) return;
  if(window.MaterialsReadinessGate && !MaterialsReadinessGate.assertGate()) return;
  if(window.SeoStrategy && !SeoStrategy.assertGate()) return;
  if(window.PreBuildWorkReport && !PreBuildWorkReport.assertBuildGate()) return;
