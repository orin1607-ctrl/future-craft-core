...
const MARKETING_URL = `${BASE}/ai-marketing-platform.html?v=v3-unified-3`;
...
async function testMarketingDirect(page, label) {
  ...
  record(`MKT-version-${label}`, 'Marketing ui-version', state.ui === 'v3-unified-3', { note: state.ui });
...
async function testMarketingIframe(page, label) {
  const iframeSrc = `${BASE}/ai-marketing-platform.html?fullscreen=1&v=v3-unified-3`;
  ...
  record(`MKT-iframe-${label}`, 'Marketing in iframe', st.hub && st.layout && st.ui === 'v3-unified-3');
}
