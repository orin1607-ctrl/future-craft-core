/**
 * Apply staff meta + import notes to Staging. Deploys claims-docs and claims-gmail.
 * node scripts/apply-claims-pack11-stage2-staging.mjs
 */
...
const SQL = join(ROOT, 'supabase/migrations/20260901140000_claims_doc_staff_meta.sql');
const OUT = join(ROOT, 'docs/audit-reports/claims-pack11-2026-09-01');
mkdirSync(OUT, { recursive: true });
if (!existsSync(SQL)) throw new Error('sql missing');

const tmpWork = join(process.env.TEMP || '/tmp', 'fcc-claims-pack11-s2');
...
try {
  ...
  for (const fn of ['claims-docs', 'claims-gmail']) {
    ...
    parts.push(String(out).slice(0, 400));
  }
  report.edge = { ok: true, output: parts.join('\n') };
  const verify = dbQuery(writeTmp(`
    SELECT json_build_object(
      ...
      'staff_note_col', (SELECT count(*) FROM information_schema.columns WHERE table_schema='public' AND table_name='claims_gmail_imports' AND column_name='staff_note'),
      'bucket_public', (SELECT public FROM storage.buckets WHERE id='claims-docs')
    );
  `));
  report.verify = { ok: true, output: String(verify).slice(0, 2000) };
} catch (e) {
...
writeFileSync(join(OUT, 'stage2-apply.json'), JSON.stringify(report, null, 2), 'utf8');
