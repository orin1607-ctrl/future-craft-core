/** Generate vehicles Row/Insert/Update snippet for types.ts (dalia-staging schema). */
const cols = [
  ['id', 'string'],
...
function block(kind) {
  const lines = cols.map(([name, type]) => {
    const opt = kind === 'Row' ? '' : '?';
    const req = kind === 'Insert' && name === 'license_plate' ? '' : opt;
